...
def insertaEvento(nombre, lugar, descripcion, fecha, conexion):
	...
	if (hay==0) or (hay==None):
		...
		devolver = obtenEventoPorID(hayEvento(nombre.strip(), cursor), cursor)
		return devolver
...
def contenidosConEvento(evento, cursor):
	devolver=[]
	if (hayEvento(evento, cursor)>0):
...
def hayContenido(contenido, cursor):
	consulta = "SELECT id FROM contenido WHERE titulo = '"+contenido.strip()+"';"
	q=cursor.execute(consulta)
	resultados = cursor.fetchall()
	if resultados is not None:
		for resultado in resultados:
...
def insertaTemasDeContenido(idContenido, arrayIdTemas, conexion):
	cursor = conexion.cursor()
	hay=hayContenido(contenido, cursor)
	if (hay==0) or (hay==None):	
